...
interface LoadingOverlayProps {
  ...
  gemsCount: number;
  /**
   * "tower" = budowa wiezy (start poziomu, licznik klejnotow),
   * "back"  = warstwa oczekiwania przy powrocie do menu / wyboru poziomu /
   *           edytora, gdzie nie ma czego liczyc — liczy sie tylko to, ze
   *           overlay jest na ekranie ZANIM zbuduje sie ciezkie tlo.
   */
  variant?: "tower" | "back";
}
  ...
  return (
    <div className={`font-freckle fixed inset-0 z-[60] flex items-center justify-center bg-gradient-to-b from-[#0e4a6e] to-[#052e1a] transition-opacity duration-300 ${visible ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
      <div className="text-center animate-in zoom-in-95 duration-300">
        {isBack ? (
          <h3 className="text-xl tracking-[0.2em] text-[#7fd4e6] mb-6 drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)]">
            {t.loadingBack}
          </h3>
        ) : (
          <h3 className="text-xl tracking-[0.2em] text-[#7fd4e6] mb-6 drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)]">
            {t.loadingTower(levelNumber)}
          </h3>
        )}
