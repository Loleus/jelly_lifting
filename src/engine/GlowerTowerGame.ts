...
//     this.bloomPass = new UnrealBloomPass(new THREE.Vector2(RENDER_WIDTH, RENDER_HEIGHT), 0.01, 0.0, 0.24);
...
//       const padMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.4, 0.1, 16), new THREE.MeshStandardMaterial({ color: "#fbbf24", emissive: "#f59e0b", emissiveIntensity: 0.6, metalness: 0.8 }));
...
//       if (lintelMat.map) { lintelMat.map.center.set(0.5, 0.5); lintelMat.map.rotation = Math.PI / 2; lintelMat.map.needsUpdate = true; }
//       if (lintelMat.normalMap) { lintelMat.normalMap.center.set(0.5, 0.5); lintelMat.normalMap.rotation = Math.PI / 2; lintelMat.normalMap.needsUpdate = true; }
...
//       if (this.playerHudTimer >= 0.1) { this.playerHudTimer = 0; this.onPlayerStateUpdate?.(this.playerState); }
//     };
//     this.animFrameId = window.requestAnimationFrame(loop);
//   }
...
//       if (arrow) arrow.position.y = Number(arrow.userData.baseY) + Math.sin(sec * 4 + i) * 0.12;
//     }
